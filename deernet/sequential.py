...
class Sequential(object):
    ...
    def fit_predict(self,x_train, y_train, x_test):
        for layer in self.layer_list:
            ...
            # print(layer_train_features.shape)
    ...
    def get_tr_neuron_feature(self,clf, x_train, y_train, x_test):
        ...
        for i,(train_index, test_index) in enumerate(self.kf.split(x_train)):
            ...
            if clf.score_func == "predict":
                neuron_features_test[:,i] = clf.predict(x_test)
            elif clf.score_func == "decision_function":
                neuron_features_test[:,i] = clf.decision_function(x_test)
            elif clf.score_func == "predict_proba":
                neuron_features_test[:,i] = clf.predict_proba(x_test)[:,0]
...
class SKNeuron(object):
    def __init__(self, clf, params=None, score_func = "predict"):
